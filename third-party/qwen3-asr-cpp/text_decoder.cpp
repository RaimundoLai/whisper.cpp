...
#define QWEN3_ASR_KQ_MASK_PAD 64
...
bool TextDecoder::load_model(const std::string & path, bool use_gpu, bool debug) {
    ...
    if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> Loading %s | use_gpu: %d\n", path.c_str(), use_gpu);
    struct gguf_context * ctx = gguf_init_from_file(path.c_str(), params);
    if (!ctx) {
        error_msg_ = "Failed to open GGUF file: " + path;
        if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> gguf_init_from_file FAILED\n");
        return false;
    }
    ...
    if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> Executing load_tensor_data\n");
    if (!load_tensor_data(path, ctx, use_gpu, debug)) {
        if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> load_tensor_data FAILED: %s\n", error_msg_.c_str());
        free_decoder_model(model_);
        gguf_free(ctx);
        if (meta_ctx) ggml_free(meta_ctx);
        return false;
    ...
    if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> Executing load_vocab\n");
    if (!load_vocab(ctx)) {
        free_decoder_model(model_);
        gguf_free(ctx);
        if (meta_ctx) ggml_free(meta_ctx);
    ...
    if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> Initializing CPU Backend\n");
    ...
    if (use_gpu) {
        if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> Initializing GPU Backend\n");
        state_.backend_gpu = ggml_backend_init_by_type(GGML_BACKEND_DEVICE_TYPE_GPU, nullptr);
        if (debug) fprintf(stderr, "[DEBUG] TextDecoder::load_model -> GPU Backend Initialized: %d\n", state_.backend_gpu != nullptr);
    } else {
        state_.backend_gpu = nullptr;
    }
...
bool TextDecoder::load_tensor_data(const std::string & path, struct gguf_context * ctx, bool use_gpu, bool debug) {
    int64_t file_size = qwen_get_file_size(path);
    if (file_size < 0) {
    ...
    void * mmap_addr = qwen_mmap(fd, file_size);
    close(fd);
    ...
    if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (decoder): Check GPU device type\n");
    ggml_backend_dev_t gpu_dev = use_gpu ? ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_GPU) : nullptr;
    bool copy_needed = false;
    if (gpu_dev) {
#if defined(__APPLE__) && defined(__aarch64__)
        if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (decoder): Device is GPU (Apple Silicon). Calling buf_from_host_ptr\n");
        model_.buffer = ggml_backend_dev_buffer_from_host_ptr(gpu_dev, data_base, total_size, max_tensor_size);
#endif
        if (!model_.buffer) {
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (decoder): Allocating VRAM for fallback deep copy\n");
            // Unify memory failed or not supported (e.g. CUDA on PC). Allocate actual VRAM buffer and set flag to deep copy.
            model_.buffer = ggml_backend_alloc_ctx_tensors_from_buft(model_.ctx, ggml_backend_dev_buffer_type(gpu_dev));
            copy_needed = true;
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (decoder): Allocated VRAM successfully? %d\n", model_.buffer != nullptr);
        } else {
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (decoder): Buffer from host ptr SUCCEEDED\n");
        }
    }
    ...
    if (!model_.buffer) {
        error_msg_ = "Failed to create backend buffer from mmap";
        qwen_munmap(mmap_addr, file_size);
    ...
    for (int64_t i = 0; i < n_tensors; ++i) {
        ...
        if (copy_needed) {
            // Tensor already has backend memory from alloc_ctx_tensors_from_buft
            // We physically copy from mmap_addr into CUDA VRAM
            ggml_backend_tensor_set(tensor, data_base + offset, 0, ggml_nbytes(tensor));
        } else {
    ...
    if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (decoder): Completed tensor copies\n");
    // We keep the CPU mapping mapped because `ctx` is still used for load_vocab,
    // and `free_decoder_model` handles the actual `qwen_munmap`.
...
struct ggml_cgraph * TextDecoder::build_graph(
    ...
    // Flash attention causal mask: [n_kv, n_tokens_padded], F16
    int n_kv = n_past + n_tokens;
    // Pad n_tokens to multiple of 32 for Metal/CUDA FlashAttention
    int n_tokens_padded = (n_tokens + QWEN3_ASR_KQ_MASK_PAD - 1) / QWEN3_ASR_KQ_MASK_PAD * QWEN3_ASR_KQ_MASK_PAD;
    struct ggml_tensor * fa_mask = ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, n_kv, n_tokens_padded);
...
bool TextDecoder::forward_with_audio(
    ...
    if (fa_mask_t) {
        int n_kv = n_past + n_tokens;
        int n_tokens_padded = fa_mask_t->ne[1];
        std::vector<ggml_fp16_t> mask_data((size_t)n_kv * n_tokens_padded);
        const ggml_fp16_t zero_f16 = ggml_fp32_to_fp16(0.0f);
        const ggml_fp16_t neginf_f16 = ggml_fp32_to_fp16(-INFINITY);
        
        // Fill first n_tokens rows
        for (int q = 0; q < n_tokens; ++q) {
            for (int k = 0; k < n_kv; ++k) {
                mask_data[k + q * n_kv] = (k <= n_past + q) ? zero_f16 : neginf_f16;
            }
        }
        // Fill padding rows with -inf
        for (int q = n_tokens; q < n_tokens_padded; ++q) {
             for (int k = 0; k < n_kv; ++k) {
...
void free_decoder_model(text_decoder_model & model) {
    ...
    if (model.mmap_addr) {
        qwen_munmap(model.mmap_addr, model.mmap_size);
