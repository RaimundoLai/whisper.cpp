...
bool GGUFLoader::load(const std::string & path, audio_encoder_model & model, bool use_gpu, bool debug) {
    ...
    if (!load_tensor_data(path, ctx, model, use_gpu, debug)) {
        free_model(model);
        gguf_free(ctx);
        if (meta_ctx) ggml_free(meta_ctx);
        return false;
...
bool GGUFLoader::load_tensor_data(const std::string & path, struct gguf_context * ctx, 
                                   audio_encoder_model & model, bool use_gpu, bool debug) {
    ...
    if (gpu_dev) {
        ...
        if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Device is GPU (Apple Silicon). Calling ggml_backend_dev_buffer_from_host_ptr\n");
        model.buffer = ggml_backend_dev_buffer_from_host_ptr(gpu_dev, data_base, total_size, max_tensor_size);
#endif
        if (!model.buffer) {
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Allocating VRAM for fallback deep copy\n");
            // Unify memory failed or not supported (e.g. CUDA on PC). Allocate actual VRAM buffer and set flag to deep copy.
            model.buffer = ggml_backend_alloc_ctx_tensors_from_buft(model.ctx, ggml_backend_dev_buffer_type(gpu_dev));
            copy_needed = true;
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Allocated VRAM successfully? %d\n", model.buffer != nullptr);
        } else {
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Buffer from host ptr SUCCEEDED (Unified memory)\n");
        }
    }
    
    // CPU Fallback
    if (!model.buffer) {
        if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Falling back to CPU buffer\n");
        model.buffer = ggml_backend_cpu_buffer_from_ptr(data_base, total_size);
        copy_needed = false;
    }
    if (!model.buffer) {
        if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: FATAL ERROR could not create backend buffer\n");
    ...
    if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Starting tensor mapping loop (copy_needed=%d)\n", copy_needed);
    for (int64_t i = 0; i < n_tensors; ++i) {
        const char * name = gguf_get_tensor_name(ctx, i);
        size_t offset = gguf_get_tensor_offset(ctx, i);
    ...
    if (debug) fprintf(stderr, "[DEBUG] load_tensor_data: Completed tensor copies\n");
    // We keep the CPU mapping mapped because `ctx` might still be used for vocab loading,
    // and `free_encoder_model` handles the actual `qwen_munmap`.
