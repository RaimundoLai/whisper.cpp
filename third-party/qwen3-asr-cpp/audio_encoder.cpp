...
bool AudioEncoder::load_model(const std::string & path, bool use_gpu, bool debug) {
    if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> GGUFLoader %s | GPU: %d\n", path.c_str(), use_gpu);
    GGUFLoader loader;
    if (!loader.load(path, model_, use_gpu, debug)) {
        error_msg_ = loader.get_error();
        if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> loader.load FAILED: %s\n", error_msg_.c_str());
        return false;
    }
    
    if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> Initializing CPU Backend\n");
    ...
    if (use_gpu) {
        if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> Initializing GPU Backend\n");
        state_.backend_gpu = ggml_backend_init_by_type(GGML_BACKEND_DEVICE_TYPE_GPU, nullptr);
        if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> GPU Backend Initialized: %d\n", state_.backend_gpu != nullptr);
    } else {
        state_.backend_gpu = nullptr;
    }
    ...
    if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> Creating Scheduler\n");
    ...
    if (debug) fprintf(stderr, "[DEBUG] AudioEncoder::load_model -> Completed successfully\n");
    return true;
}
