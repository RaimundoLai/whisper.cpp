...
#define QWEN3_FA_KQ_MASK_PAD 64
...
bool ForcedAligner::load_model(const std::string & model_path, bool use_gpu, bool debug) {
    if (debug) fprintf(stderr, "[DEBUG] ForcedAligner::load_model -> Initializing\n");
    ...
    if (debug) fprintf(stderr, "[DEBUG] ForcedAligner::load_model -> Executing load_tensor_data\n");
    if (!load_tensor_data(model_path, ctx, use_gpu, debug)) {
        free_forced_aligner_model(model_);
        gguf_free(ctx);
        if (meta_ctx) ggml_free(meta_ctx);
        return false;
    ...
    if (debug) fprintf(stderr, "[DEBUG] ForcedAligner::load_model -> Initializing CPU Backend\n");
    ...
    if (use_gpu) {
        if (debug) fprintf(stderr, "[DEBUG] ForcedAligner::load_model -> Initializing GPU Backend\n");
...
bool ForcedAligner::load_tensor_data(const std::string & path, struct gguf_context * ctx, bool use_gpu, bool debug) {
    ...
    if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (aligner): Check GPU device type\n");
    ...
    if (gpu_dev) {
        ...
        if (!model_.buffer) {
            ...
            if (debug) fprintf(stderr, "[DEBUG] load_tensor_data (aligner): Buffer from host ptr SUCCEEDED\n");
        }
    }
    ...
    // and `free_forced_aligner_model` handles the actual `qwen_munmap`.
...
struct ggml_cgraph * ForcedAligner::build_decoder_graph(
    ...
    // Pad n_tokens to multiple of 64 for Metal/CUDA FlashAttention
    int n_tokens_padded = (n_tokens + QWEN3_FA_KQ_MASK_PAD - 1) / QWEN3_FA_KQ_MASK_PAD * QWEN3_FA_KQ_MASK_PAD;
    struct ggml_tensor * causal_mask = ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, n_tokens, n_tokens_padded);
...
bool ForcedAligner::forward_decoder(
    ...
    if (mask_t) {
        int n_tokens_padded = mask_t->ne[1];
        std::vector<ggml_fp16_t> mask_data((size_t)n_tokens * n_tokens_padded);
        ...
        for (int q = n_tokens; q < n_tokens_padded; ++q) {
             for (int k = 0; k < n_tokens; ++k) {
                mask_data[k + q * n_tokens] = neginf_f16;
             }
        }
...
std::vector<int32_t> ForcedAligner::tokenize_with_timestamps(
    ...
    if (language == "korean" && !model_.ko_dict.empty()) {
        ...
        while (i < text.size()) {
            ...
            if (c < 0x80) {
                // ASCII: read until whitespace or high-byte char
                while (i < text.size()) {
                    unsigned char curr = static_cast<unsigned char>(text[i]);
                    if (curr >= 0x80 || curr == ' ' || curr == '\t' || curr == '\n' || curr == '\r') break;
                    ++i;
                }
            } else {
                // UTF-8 sequence (treat each multi-byte char as a word for CJK)
                int len = 1;
                if ((c & 0xE0) == 0xC0) len = 2;
                else if ((c & 0xF0) == 0xE0) len = 3;
                else if ((c & 0xF8) == 0xF0) len = 4;
